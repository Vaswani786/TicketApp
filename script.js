    ...
    if (flag==false) appendTicket(ticketInfoObject);
      ...
      $.each(allTickets, function(i, ticket) {
        if (ticket.ticketId == ticketInfoObject.ticketId) {
          ticket.ticketValue = ticketInfoObject.ticketValue;
          ticket.ticketFilter = ticketInfoObject.ticketFilter;
...
function appendTicket(ticketInfoObject) {
  ...
  deleteTicketBtn.addEventListener("click", (e) => {
    ...
    let updatedTicket = allTickets.filter((ticketObject) => {
...
let penedit = ticketDiv.querySelector(".ticket-edit");
penedit.addEventListener("click", () => {
  openModal.click();
  let tickeinformation = $(".ticket-text");
  tickeinformation.text(ticketValue);
  filterselected = $(".selected-filter").attr("class").split(" ")[1];
  flag = true,ticketchecker = ticketId;
});
